...
ContraDito — portal de transparência política que cruza discursos de parlamentares com votos em plenário, gerando um Score de Coerência via IA.
...
- Página: HTML5 estático (`docs/productivity/index.html`)
...
- Paleta de cores (UnB institucional):
  - Azul escuro: #011d3a — header, séries "Abertas", barras empilhadas abertas.
  - Verde: #550e01 — séries "Fechadas", barras empilhadas fechadas.
  - Azul médio: #242424 — histograma de commits.
  - Dourado: #f3eabb — barras de co-autores.
- Dados: arquivo docs/productivity/metrics.json gerado pelo GitHub Action.
...
- Deploy da página: GitHub Pages servindo a pasta docs/productivity/.
...
- Não altere nenhum arquivo fora de docs/productivity/ e .github/workflows/metrics.yml sem permissão.
...
Contexto: A equipe (Squad 03) e a professora da disciplina MDS/UnB precisam visualizar a produtividade do projeto VeritasIA ao longo do tempo. A página é acessada via GitHub Pages e mostra gráficos e rankings atualizados semanalmente de forma automática.
...
4. Mapa de calor — Horário dos commits por dia da semana
   ...
   - Cor: gradiente de #74a5b6 (zero commits) a #003366 (máximo).
...
6. Ranking — Top committers
   ...
   - Adicione uma coroa em cima da pessoa que mais tiver commits.
   - Pegue as fotos de pessoa e coloque nesse ranqueamento
...
1. O arquivo docs/productivity/index.html abre corretamente no navegador sem servidor local (protocolo file:// ou via GitHub Pages).
2. O arquivo docs/productivity/metrics.json é gerado pelo workflow e contém todas as seções: issues_per_week, commit_message_histogram, coauthors_per_week, commit_heatmap, top_committers, top_pr_authors, top_issue_contributors.
...
1. Criar script coletor (`docs/productivity/collect_metrics.py`)
   - Depende de: —
   - Pronto quando:
     - O script aceita variáveis de ambiente GITHUB_TOKEN e GITHUB_REPOSITORY.
     - Gera docs/productivity/metrics.json com schema correto.
...
3. Criar página HTML (`docs/productivity/index.html`)
